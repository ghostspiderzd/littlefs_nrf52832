	...
	unsigned char i = 0;
		...
		//if((rxflag == 1) && (gCmdflag == 1))
		{
			...
			if(spi1rxbuff[0] == 0xaa)
			{
				...
				/*
					if((DataPack.PPGData.Value[adc_value_Big_buf.Byte_cnt] != 0x11) || \
					   (DataPack.PPGData.Value[adc_value_Big_buf.Byte_cnt +1] != 0x22) || \
					   (adc_value_Big_buf.L1Value[adc_value_Big_buf.Byte_cnt] != 0x33) || \
					   (adc_value_Big_buf.L1Value[adc_value_Big_buf.Byte_cnt+1] != 0x44))
						{
						gerrcnt++;
					   }
				*/	
...
int main(void)
{
		...
		unsigned char testspitx[] = "hello world";
		unsigned char testspirx[12];
	  ...
	  /*ÉèÖÃLEDÖ¸Ê¾µÆ*/
		...
		//flash_dev_spi_init();
		//while(W25Q80_ReadID() != 0xEF4020)
		{
		//	nrf_delay_ms(500);
		}

		//spi_flash_Write_NoCheck(testspitx,0x080103,11);
		//spi_flash_read_data_52832(testspirx,0x080103,11);
		
		//spi_flash_littlefs_test();
		

		//spi_flash_write_page(testspitx,0,11);
		//spi_flash_read_data_52832(testspirx,1411,10);
		//spi_flash_erase_addr(0);
		//spi_flash_read_data_52832(testspirx,0,11);
