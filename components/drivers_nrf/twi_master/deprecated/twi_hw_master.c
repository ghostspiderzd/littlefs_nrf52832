...
#include "nrf_gpio.h"
#include "BLE_SET.h"
extern LED_STATE m_stable_led_state ;
...
static bool twi_mlx90615_write(uint8_t * data, uint8_t data_length, bool issue_stop_condition)
{
    uint32_t timeout = MAX_TIMEOUT_LOOPS; /* max loops to wait for EVENTS_TXDSENT event*/
...
/** @brief Function for read by twi_master.
 */
static bool twi_mlx90615_read(uint8_t * data, uint8_t data_length, bool issue_stop_condition)
{
    uint32_t timeout = MAX_TIMEOUT_LOOPS; /* max loops to wait for RXDREADY event*/
 ...
 * @retval true Bus is clear.
 */
static bool twi_mlx90615_clear_bus(void)
{
    uint32_t twi_state;
    bool     bus_clear;
    uint32_t clk_pin_config;
    ...
    clk_pin_config = \
        NRF_GPIO->PIN_CNF[TWI_MLX90615_CLOCK_PIN_NUMBER];
    NRF_GPIO->PIN_CNF[TWI_MLX90615_CLOCK_PIN_NUMBER] =      \
        (GPIO_PIN_CNF_SENSE_Disabled  << GPIO_PIN_CNF_SENSE_Pos) \
      | (GPIO_PIN_CNF_DRIVE_S0D1    << GPIO_PIN_CNF_DRIVE_Pos)   \
      | (GPIO_PIN_CNF_PULL_Pullup   << GPIO_PIN_CNF_PULL_Pos)    \
    ...
    data_pin_config = \
        NRF_GPIO->PIN_CNF[TWI_MLX90615_DATA_PIN_NUMBER];
    NRF_GPIO->PIN_CNF[TWI_MLX90615_DATA_PIN_NUMBER] =       \
        (GPIO_PIN_CNF_SENSE_Disabled  << GPIO_PIN_CNF_SENSE_Pos) \
      | (GPIO_PIN_CNF_DRIVE_S0D1    << GPIO_PIN_CNF_DRIVE_Pos)   \
      | (GPIO_PIN_CNF_PULL_Pullup   << GPIO_PIN_CNF_PULL_Pos)    \
    ...
    NRF_GPIO->PIN_CNF[TWI_MLX90615_CLOCK_PIN_NUMBER] = clk_pin_config;
    NRF_GPIO->PIN_CNF[TWI_MLX90615_DATA_PIN_NUMBER]  = data_pin_config;
...
bool twi_mlx90615_init(void)
{
    ...
    */
    ...
    NRF_TWI0->EVENTS_RXDREADY = 0;
    NRF_TWI0->EVENTS_TXDSENT  = 0;
    NRF_TWI0->PSELSCL         = TWI_MLX90615_CLOCK_PIN_NUMBER;
    NRF_TWI0->PSELSDA         = TWI_MLX90615_DATA_PIN_NUMBER;
...
bool twi_mlx90615_transfer(uint8_t   address,
                         uint8_t * data,
                         uint8_t   data_length,
                         bool      issue_stop_condition)
{
    bool transfer_succeeded = false;
    if (data_length > 0 && twi_mlx90615_clear_bus())
    {
        NRF_TWI0->ADDRESS = (address >> 1);
			
        if ((address & TWI_READ_BIT))
        {
            transfer_succeeded = twi_mlx90615_read(data, data_length, issue_stop_condition);
        }
        else
        {
            transfer_succeeded = twi_mlx90615_write(data, data_length, issue_stop_condition);
        }
    }
    return transfer_succeeded;
}
