...
*/
bool mpu6050_register_write(unsigned char register_address, const unsigned char value);
...
#define _accRAW_TO_TLV(x) ((((int)(x))>>9)+(((int)(x))>>10)+(((int)(x))>>13))     /*由于MPU6050 加速度传感器16位分辨率设定正负2G量程则TLV规定每G变化是50*/
...
bool mpu6050_register_write_len(unsigned char addr,unsigned char register_address, unsigned char len,unsigned char *buf);
bool mpu6050_register_read_len(unsigned char addr,unsigned char register_address, unsigned char number_of_bytes,unsigned char * destination );
